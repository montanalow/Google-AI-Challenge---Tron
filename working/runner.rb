...
maps = maps.select{|m| m.include? ARGV[0] } if ARGV[0]
bots = bots.select{|b| b != '.' && b != '..' }

wins = 0
losses = 0
collisions = 0
draws = 0
m_results = Hash.new {|hash,key| hash[key] = {:wins => 0, :losses => 0, :draws => 0, :collisions => 0, :ration => 0} }
maps.each do |m|
  bots.each do |b|
    next if m == "huge-room.txt" || m.include?("test")
    debug = `java -jar engine/Tron.jar maps/#{m} "./zug_zug" "java -jar example_bots/#{b}" 0 1`
    result = debug.split("\n").last
    ...
    debug = `java -jar engine/Tron.jar maps/#{m} "java -jar example_bots/#{b}" "./zug_zug" 0 1`
    result = debug.split("\n").last
