...
zz::bot::bot( )
: _min_iterations(4), _script( ), _scripted_move( _script.end( ) ), _map( ), _apocalyptic( "./maps/apocalyptic.txt" ), _big_room( "./maps/big_room.txt" ), _center( "./maps/center.txt" ), _divider( "./maps/divider.txt" ), _empty_room( "./maps/empty-room.txt"), _joust( "./maps/joust.txt" ), _keyhole( "./maps/keyhole.txt" ), _medium_room( "./maps/medium_room.txt" ), _playground( "./maps/playground.txt" ), _quadrant( "./maps/quadrant.txt" ), _ring( "./maps/ring.txt" ), _toronto( "./maps/toronto.txt" ), _trix( "./maps/trix.txt" ), _u( "./maps/u.txt" ) {
}
...
void zz::bot::move( ) {
  ...
  int max;
  int maxes;
  int iterations = _min_iterations;
  clock_t move_clock = clock();
  do {
    max = _map.minimax( iterations );
    ++iterations;
    maxes = 0;
    for( int i = 0; i < 4; ++i ) { if( _map._mins[i] == max ) ++maxes; }
  } while( false || maxes > 1 && max < 512 && (float) (clock() - move_clock) / CLOCKS_PER_SEC < 0.1f );

  std::map<zz::map::direction, int> directions_minimax_scores;
  for( int i = 0; i < 4; ++i ) {
    if( _map._mins[i] == max ) {
      directions_minimax_scores[i] = _map._mins[i];
    }
  }
...
bool zz::bot::running_script( ) {
  ...
  if( _map == _apocalyptic ) {
    ...
    //    }
    //    return true;
  } else if( _map == _empty_room ) {
    _min_iterations = 4;
    if( _map.player_one( ) == zz::map::location( 1, 1 ) ) {
      int script[1] = { zz::map::south };
      set_script( script, 1 );
    } else {
      int script[1] = { zz::map::north };
      set_script( script, 1 );
    }
    return true;
  } else if( _map == _joust ) {
    ...
    _min_iterations = 11;
    if( _map.player_one( ) == zz::map::location( 11, 11 ) ) {
      int script[1] = { zz::map::north };
      set_script( script, 1 );
    } else {
      int script[1] = { zz::map::south };
    ...
    _min_iterations = 2;
    if( _map.player_one() == zz::map::location(1,1) ) {
      int script[1] = {zz::map::east};
      set_script( script, 1 );
    } else {
      int script[1] = {zz::map::west};
      set_script( script, 1 );
    }
    return true;
  } else if( _map == _u ) {
    ...
    for( int j = 0; j < 4; ++j ) {
      ...
      if( collision || ( player_two_dies && player_one_dies ) ) {
        ...
        if( iterations > 0 ) {
          ...
          result = std::min( 1024, std::max( -1024, (map.area_available_from( player_one ) - map.area_available_from( player_two )) * 256));
