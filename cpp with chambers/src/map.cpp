...
zz::map::~map( ) {
  for( zz::map::chambers::iterator it = _chambers.start(); it != _chambers.end(); ++it ){
    delete *it;
  }
}
...
const bool zz::map::is_wall( zz::map::coordinate x, zz::map::coordinate y ) const {
  return x < 0 || y < 0 || x >= width() || y >= height() || is_wall( x + width() * y );
}
...
void zz::map::adjust_chambers( const zz::map::location& location, int building_chamber ) const {
//  fprintf( stderr, "set_chambers(%d,%d)\n", location.x(), location.y() );
//  // flag newly created hallways
//
//  for( int i = 0; i < 4; ++i ) {
//    zz::map::location neighbor = location.adjacent( i );
//    int neighbor_index = neighbor.x( ) + width() * neighbor.y( );
//    if( is_wall( neighbor_index ) ) continue;
//
//    if( _count_walls[neighbor_index] == 3 && !is_chamber( neighbor ) ) {
//      zz::map::chamber *chamber = new chamber( _next_chamber_id++ );
...
//      if( outgoing_connections[came_from_square[current_index]] ) {
//        // we just stepped through a doorway into a new open chamber
//        chamber = ++count_chambers;
//        incoming_connections[current_index] = squares_chambers[came_from_square[current_index]];
//        outgoing_connections[current_index] = chamber;
//      } else {
//        // we stepped from an open chamber into an adjacent open square
//        chamber = squares_chambers[came_from_chamber[current_index]];
//        for( int i = 0; i < 4; ++i ) {
//          // if there are any other chambers next to this one, that are not doorways, make a doorway from here to there
//          adjacent_chamber = squares_chambers[neighbor_index];
//          zz::map::location neighbor = current.adjacent( i );
//          int neighbor_index = neighbor.x( ) + width( ) * neighbor.y( );
//          if( adjacent_chamber != chamber ) {
//            incoming_connections[current_index] = adjacent_chamber;
//            outgoing_connections[current_index] = chamber;
//            break;
//          }
//        }
//      }
//    } else {
//      // this is a new doorway between chambers, it counts as it's own chamber
//      chamber = ++count_chambers;
//      incoming_connections[current_index] = squares_chambers[came_from_square[current_index]];
//      outgoing_connections[current_index] = chamber;
//    }
//    squares_chambers[current_index] = chamber;
//    chambers_sizes[chamber]++;
//  }
//
//
//
//
//    if( !building_hallway ) {
//      int hallway = is_hallway( neighbor );
//      if( count_walls == 3 ) {
//        if( hallway ) {
//          fprintf( stderr, "  neighbor(%d,%d): 3 walls existing hallway\n", neighbor.x(), neighbor.y() );
//          // rebuilding a hallway we just cleared
//          set_is_hallway( neighbor, hallway );
//        } else {
//          fprintf( stderr, "  neighbor(%d,%d): 3 walls new\n", neighbor.x(), neighbor.y() );
//          // start building a new hallway
//          _locations[neighbor.x() + width() * neighbor.y()] = ++_count_hallways;
//          set_is_hallway( neighbor, _count_hallways );
//        }
//      } else if( hallway ) {
//        fprintf( stderr, "  neighbor(%d,%d): clearing\n", neighbor.x(), neighbor.y() );
//        _locations[neighbor.x() + width() * neighbor.y()] = 0;
////        set_is_hallway( neighbor, building_hallway );
//      }
//    } else {
//      if( count_walls == 3 ){
//        fprintf( stderr, "  neighbor(%d,%d): 3 walls dead end\n", neighbor.x(), neighbor.y() );
//        // dead end
//        _locations[neighbor.x() + width() * neighbor.y()] = building_hallway;
//      } else if( count_walls == 2 && is_hallway( neighbor ) != building_hallway ){
//        fprintf( stderr, "  neighbor(%d,%d): 2 walls building\n", neighbor.x(), neighbor.y() );
//        _locations[neighbor.x() + width() * neighbor.y()] = building_hallway;
//        set_is_hallway( neighbor, building_hallway );
//      } else {
//        fprintf( stderr, "  neighbor(%d,%d): opened up\n", neighbor.x(), neighbor.y() );
//      }
//    }
//  }
}

//void zz::map::set_count_walls( const zz::map::location& location,  ) const {
//}
...
//    zz::map::location neighbor = location.adjacent( i );
//    if( value ) {
//      ++_count_walls[neighbor.x( ) + width() * neighbor.y( )];
//    } else {
//      --_count_walls[neighbor.x( ) + width() * neighbor.y( )];
//    }
//  }
//  set_is_hallway( location );
};

int zz::map::is_chamber( const zz::map::location& location ) const {
  if( _locations[location.x() + width() * location.y()] > 0 && _locations[location.x() + width() * location.y()] % 2 == 0 ){
    return _locations[location.x() + width() * location.y()];
  }
  return 0;
}

int zz::map::is_door( const zz::map::location& location ) const {
  if( _locations[location.x() + width() * location.y()] > 0 && _locations[location.x() + width() * location.y()] % 2 == 1){
    return _locations[location.x() + width() * location.y()];
  }
  return 0;
}
...
int zz::map::territory( ) const {
  ...
  while( current_todo < todo_end ) {
    ...
    for( int i = 0; i < 4; ++i ) {
      ...
      int neighbor_index = neighbor.x( ) + width( ) * neighbor.y( );
      if( !counted_locations[neighbor_index] && !is_wall(neighbor_index) ) {
        int neighbor_distance = distance + 1;
        player_two_distances[neighbor_index] = neighbor_distance;
        counted_locations[neighbor_index] = true;
        ( *todo_end++ ) = neighbor_index;
      }
    }
...
int zz::map::chamber_territory() const {
  int todo[width() * height()];
  int counted_locations[width() * height()];

  int came_from_square[width() * height()];
  int doors[width() * height()];
  std::vector<zz::map::chamber> chambers;
  ...
  while( current_todo < todo_end ) {
    ...
    for( int i = 0; i < 4; ++i ) {
      ...
      zz::map::location neighbor = current.adjacent( i );
      int neighbor_index = neighbor.x( ) + width( ) * neighbor.y( );
      if( is_wall( neighber_index ) ) {
        ++count_walls;
      } else if( !counted_locations[neighbor_index] ) {
        counted_locations[neighbor_index] = true;
        came_from_square[neighbor_index] = current_index;
        ( *todo_end++ ) = neighbor_index;
      }
    }
    if( count_walls < 2 ) {
      // this is not a doorway
      doors[current_index] = false;
      if( doors[came_from_square[current_index]] ) {
        // we just stepped through a doorway into a new open chamber
        chambers.push_back( zz::map::chamber( chambers.size() ) );
        zz::map::chamber& chamber = chambers.back();
        _locations[current_index] = chamber.id();
        chamber.add_connection( chambers[_locations[came_from_square[current_index]]] );
        chambers[_locations[came_from_square[current_index]]].add_connection( chamber );
        chamber.inc_size();
      } else {
        // we stepped from an open chamber into an adjacent open square
        zz::map::chamber& chamber = chambers[_locations[came_from_square[current_index]]];
        chamber.inc_size();
        for( int i = 0; i < 4; ++i ) {
          // if there are any other chambers next to this one, add a connection
          zz::map::location neighbor = current.adjacent( i );
          int neighbor_index = neighbor.x( ) + width( ) * neighbor.y( );
          if( !is_wall( neighbor_index) && _locations[neighbor_index] != chamber.id() ) {
            chamber.add_connection( chambers[_locations[nieghbor_index]] );
            chambers[_locations[nieghbor_index]].add_connection( chamber );
          }
        }
      }
    } else {
      // this is a new doorway between chambers, it counts as it's own chamber
      doors[current_index] = true;
      chambers.push_back( zz::map::chamber( chambers.size() ) );
      zz::map::chamber& chamber = chambers.back();
      chamber.inc_size();
      _locations[current_index] = chamber.id();
  ...
  bool merged = false;
  do {
    for( std::vector<zz::map::chamber>::iterator chamber = chambers.begin(); chamber != chambers.end(); ++chamber ){
      if( (*chamber).connections().size() == 1 ){
        // merge dead ends back up into their parent chambers
        zz::map::chamber& parent = (*chamber).connections().first();
        parent.size() += (*chamber).size();
        chamber.remove_connection( parent );
        parent.remove_connection( chamber );
        chambers[chamber.id()] = parent;
        merged = true;
      } else if ( (*chamber).connections().size() == 2 ) {
        // merge hallways to their parent
        zz::map::chamber& parent = (*chamber).connections().first();
        parent.size() += (*chamber).size();
        chamber.remove_connection( parent );
        parent.remove_connection( chamber );
        parent.add_connection( chamber.connections.last() );
        chamber.connections.last().remove_connection( chamber );
        chamber.connections.last().add_connection( parent );
        chambers[chamber.id()] = parent;
        merged = true;
      } else {
        std::sort( chamber.connections().begin(), chamber.connections().end() );
        std::vector<zz::map::chamber>::iterator current = chamber.connections().begin();
        std::vector<zz::map::chamber>::iterator next = current;
        while( ++next != chamber.connections().end() ){
          if( (*current) == (*next) ){
            // there are 2 connections between this and another chamber, so they are the same chamber
            (*current).size() += chamber.size();
            for( std::vector<zz::map::chamber>::iterator connected_chamber = chamber.connections().begin(); connected_chamber != chamber.connections().end(); ++connected_chamber ){
              if( *connected_chamber != *current) {
                (*connected_chamber).add_connection( *current );
                (*current).add_connection( *connected_chamber );
              }
              (*connected_chamber).remove_connection( *chamber );
            }
            merged = true;
            chambers[chamber.id()] = *current.id();
            break; // this chamber is merged in now
          }
          ++current;
        }
      }
...
int zz::map::find_max_path( const zz::map::chamber chamber, v ) {
  if( chamber.unvisited)
}
