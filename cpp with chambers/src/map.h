...
namespace zz {
  ...
  public:
    ...
    class vertex {
    private:
       zz::graph::vertexes _neighbors;
    };

    graph();
    vertex* add_vertex( vertex* parent );
    vertex* add_egde( vertex* left, vertex* right );

  private:
    zz::graph::vertexes _vertexes;
  };
  ...
  public:
    ...
    /**************************************************************************
     * Map chambers are interesting critters
     **************************************************************************/
    class chamber;
    typedef std::list<zz::map::chamber&> chambers;

    class chamber {
    public:
      chamber( int id ) : _id( id ), _size( 0 ), _visited( false ) {};
      int id(){ return _id; }
      add_connection( zz::map::chamber& chamber ) { _connections.push_back( chamber ) } ;
      remove_connection( zz::map::chamber& chamber ) { _connections.remove( chamber ) } ;
      merge( zz::map::chamber& chamber );
      chambers& connections(){return _connections;};
      int size(){ return _size; };
      bool operator == ( const zz::map::chamber& chamber ){ return _id == other._id; };
      bool operator != ( const zz::map::chamber& chamber ){ return !(*this == other); };
    private:
      int _id;
      bool _visited;
      int _size;
      chambers _connections;
    };
  ...
  private:
    ...
    void set_is_hallway( const zz::map::location& location, int building_hallway = 0 ) const;
...
//    mutable int _count_walls[MAP_SIZE_MAX]; // sometimes we need to muck with where walls are, so that we don't have to deep copy, when we just need 1 wall changed
  };
};
